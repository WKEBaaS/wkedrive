...
export const classesInDbo = dbo.table('classes', {
	id: varchar({ length: 21 }).default(sql`nanoid()`).primaryKey().notNull(),
	entityId: integer('entity_id'),
...
export const invitationType = pgEnum('invitation_type', ['INVITATION', 'REQUEST']);
export const invitationStatus = pgEnum('invitation_status', ['PENDING', 'ACCEPTED', 'DECLINED', 'EXPIRED']);
...
export const coInDbo = dbo.table('co', {
	...
	oid: uuid().notNull(),
	rank: smallint().generatedAlwaysAsIdentity({
		name: 'dbo.co_rank_seq',
		startWith: 100,
		increment: 1,
		minValue: 1,
		maxValue: 32767,
